...
\item Fun with conditional independence.  Suppose that $X\in \{1,...,K\} $ is a discrete random variable, and let $\epsilon_1$ and $\epsilon_2$ be realizations of two other random variables $E_1$ and $E_2$, such that $E_1=\epsilon_1$ and $E_2=\epsilon_2$. We want to calculate the flowing probability: $P(X|\epsilon_1,\epsilon_2)= P(X=1|\epsilon_1,\epsilon_2)\dots  P(X=K|\epsilon_1,\epsilon_2)$. (Hint: use Bayes rule.)
  ...
  \item Derive the following conditional posterior distribution, $p(\beta_1|Y,X,\beta_1$ and $p(\beta_2|Y,X,\beta_1)$. Hing: Conditioned on $\beta_1$ we treat $\beta_1$ as known quantity. This means that we can transform the original model as
  \begin{align}
...
\begin{enumerate}
  ...
  \item Scrape data for the {\ul cities} of New York, Chicago, and Los Angeles, for the period 2017-2020. Be careful with name variations, you'll see some times ``New York'', appear as ``New York'', ``New York, NY'', or ``New York, New York''. Make sure you include all name variations. 
  \end{enumerate}  
